...
* We can write the expected test MSE as
  ...
  $$
  MSE = E_{x, y, D}\left[(y - \hat f_{\!D}(x))^2\right]
  $$
...
* Taken from http://www.cs.cornell.edu/courses/cs4780/2018fa/lectures/lecturenote12.html

* To make it clear that the expectation is over $x, y$ and $D$, let's write

  $$
  \begin{equation}
    E_{x, y, D}\left[(y - \hat f_{\!D}(x))^2\right]
  \end{equation}
  $$
...
* Now, we expand the binomial and distribute the expectation:
  ...
  $$
  \begin{gather}
    E_{x, y, D}\left[(y - E(\hat f_{\!D}(x)))^2\right] 
    + E_{x, y, D}\left[(E(\hat f_{\!D}(x)) + \hat f_{\!D}(x))^2\right] \\
    {} + 2E_{x, y, D}\left[(y - E(\hat f_{\!D}(x))) \cdot
        (E(\hat f_{\!D}(x)) + \hat f_{\!D}(x)) \right]
  \end{gather}
  $$
...
$$
\begin{align*}
  & E_{x, y, D}\left[(y - E(\hat f_{\!D}(x))) \cdot
        (E(\hat f_{\!D}(x)) + \hat f_{\!D}(x)) \right]
\end{align*}
$$
